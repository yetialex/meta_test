...
func (app *Handlers) CreateDirectory(params core.CreateDirectoryParams) middleware.Responder {
...
func (app *Handlers) DeleteDirectory(params core.DeleteDirectoryParams) middleware.Responder {
	conn, err := app.pool.Acquire(context.Background())
	if err != nil {
		app.logger.Error("Unable to acquire a database connection", zap.Error(err))
		return common.NewInternalServerError().WithPayload(&models.ErrorResponse{
...
func (app *Handlers) GetDirectory(params core.GetDirectoryParams) middleware.Responder {
	conn, err := app.pool.Acquire(context.Background())
	if err != nil {
		app.logger.Error("Unable to acquire a database connection", zap.Error(err))
		return common.NewInternalServerError().WithPayload(&models.ErrorResponse{
...
func (app *Handlers) UpdateDirectory(params core.UpdateDirectoryParams) middleware.Responder {
	conn, err := app.pool.Acquire(context.Background())
	if err != nil {
		app.logger.Error("Unable to acquire a database connection", zap.Error(err))
		return common.NewInternalServerError().WithPayload(&models.ErrorResponse{
